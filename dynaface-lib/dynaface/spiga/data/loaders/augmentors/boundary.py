...
class AddBoundary(object):
    ...
    def get_dataset_boundaries(self, landmarks, mask_lnd):
        ...
        if self.num_landmarks == 68:
            ...
            boundaries["lower_left_eyelid"] = lower_left_eyelid[
                lower_left_eyelid_mask > 0
            ]
            ...
            boundaries["lower_right_eyelid"] = lower_right_eyelid[
                lower_right_eyelid_mask > 0
            ]
            upper_outer_lip = landmarks[48:55]
            boundaries["upper_outer_lip"] = upper_outer_lip[mask_lnd[48:55] > 0]
            lower_outer_lip = np.array(
                [landmarks[i] for i in [48, 59, 58, 57, 56, 55, 54]]
            )
            lower_outer_lip_mask = np.array(
                [mask_lnd[i] for i in [48, 59, 58, 57, 56, 55, 54]]
            )
            ...
            boundaries["lower_left_eyebrow"] = np.array(
                [landmarks[i] for i in [33, 41, 40, 39, 38]]
            )
