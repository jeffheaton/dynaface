...
from facial_analysis.spiga.data.loaders.augmentors.modern_posit import PositPose
from facial_analysis.spiga.data.loaders.augmentors.heatmaps import Heatmaps
from facial_analysis.spiga.data.loaders.augmentors.boundary import AddBoundary
from facial_analysis.spiga.data.loaders.augmentors.landmarks import (
    HorizontalFlipAug,
    RSTAug,
    OcclusionAug,
    LightingAug,
...
def get_transformers(data_config):
    ...
    if "flip" in aug_names:
        augmentors.append(
            HorizontalFlipAug(
                data_config.database.ldm_flip_order, data_config.hflip_prob
            )
        )
    if "rotate_scale" in aug_names:
        augmentors.append(
            RSTAug(
                data_config.angle_range,
                data_config.scale_min,
                data_config.scale_max,
                data_config.trl_ratio,
            )
        )
    if "occlusion" in aug_names:
        augmentors.append(
            OcclusionAug(
                data_config.occluded_min_len,
    ...
    if "lighting" in aug_names:
        augmentors.append(
            LightingAug(data_config.hsv_range_min, data_config.hsv_range_max)
        )
    ...
    augmentors.append(
        TargetCropAug(
            data_config.image_size, data_config.ftmap_size, data_config.target_dist
        )
    )
    # Opencv style
    ...
    if "heatmaps2D" in aug_names:
        augmentors.append(
            Heatmaps(
                data_config.database.num_landmarks,
                data_config.ftmap_size,
                data_config.sigma2D,
                norm=data_config.heatmap2D_norm,
            )
        )
    ...
    if "boundaries" in aug_names:
        augmentors.append(
            AddBoundary(
                num_landmarks=data_config.database.num_landmarks,
