...
Subclass `FaceMeasureBase` in `facial_dll/Measures/`, declare its `MeasureItemInfo` items in the constructor (mirrors dynaface-lib's per-field enable granularity), and implement `Label` and `Calc(FaceMeasureContext ctx, bool render = true)`. Register the new class in `FacialDllConsole/Program.cs` `BuildMeasures()`. Use `ctx.Measure`, `ctx.MeasurePolygon`, and `ctx.AddHeader`/`ctx.AddValue`/`ctx.Values` for output. See `facial_dll/CLAUDE.md` for the full contract.
